...
class pdeSABR {
public:
  pdeSABR(double alpha, double beta, double nu, double rho, double eps,
          double shift, double maturity, double F, double Fmin, double Fmax,
          int sizeX, int sizeT)
      : eps_(eps), alpha_(alpha), beta_(beta), rho_(rho), nu_(nu), f_(F),
        shift_(shift), T_(maturity), Fmax_(Fmax), Fmin_(Fmin), sizeX_(sizeX),
        sizeT_(sizeT) {
    if (beta_ == 0)
      type_ = "Normal";
    else {
