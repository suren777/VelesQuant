...
public:
  ShortRate1FPDE(double R0, double kappa, double alpha, double beta,
      ...
      : R0_(R0), kappa_(kappa), alpha_(alpha), beta_(beta), gamma_(gamma),
        timeSigmas_(timeSigmas), sigmas_(sigmas), timeThetas_(timeThetas),
        thetas_(thetas) {
    in_calibration_ = false;
    buildGrid();
  };

  ShortRate1FPDE(std::vector<double> timeDFs, std::vector<double> DFs,
  ...
  void change_grid_factor(double Rmax, double factor) {
    buildGrid(Rmax, factor);
  };
