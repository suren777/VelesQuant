...
{
public:
	HWPDE(double R0, double kappa, std::vector<double> timeSigmas, std::vector<double> sigmas, 
		std::vector<double> timeThetas, std::vector<double> thetas)
		: R0_(R0), kappa_(kappa), timeSigmas_(timeSigmas), sigmas_(sigmas), 
		timeThetas_(timeThetas), thetas_(thetas) {buildGrid();sigmas0_=sigmas_; kappa0_=kappa_;};

	HWPDE(double kappa, std::vector<double> timeSigmas, std::vector<double> sigmas, 
		std::vector<double> timeDF, std::vector<double> DF)
		...
		timeDFs_(timeDF),DFs_(DF)
	{
		...
		buildGrid();
		for (int i = 0; i< timeThetas_.size();i++) thetas_.push_back(.001);
		termStructureCalibrator();
		sigmas0_=sigmas_; 
		kappa0_=kappa_;
	};

	HWPDE(std::vector<double> timeDFs, std::vector<double> DFs, std::vector<defSwap> quoteSwap)
		: timeDFs_(timeDFs), DFs_(DFs), quoteSwap_(quoteSwap)
	{
		...
		for(int i=0; i<quoteSwap_.size(); i++)
		{
		...
		for(std::list<double>::iterator it=Times.begin(); it!=Times.end(); ++it)
			timeSigmas_.push_back(*it);
		sigmas_.resize(timeSigmas_.size(), 0.003);sigmas0_=sigmas_;kappa0_=kappa_;
		buildGrid();
	...
	void   calibratorBootStrap(std::vector<double> timeDFs, std::vector<double> DFs, std::vector<defSwap> swapQuotes);
	...
	double pricingSwaption(double Expiry, double Tenor, double Strike, double PayFrequency);
	...
	std::vector<double> simulationHWPDE(std::vector<double> timePoints) const;
...
private:
	...
	void objFcnCalibration(int m, int n, double* x, double* fvec, int* iflag, double* lb, double* ub);
	...
	double swaptionValueATM(double Expiry, double Tenor, double PayFrequency, double SwapRate, double VolATM);
	...
	double dobjKappa(double a, const std::vector<double> &ivr, const std::vector<double> &br, const std::vector<int> &ind);
	//Mesh initialisation
