...
protected:	
	...
	void oneStepForward(std::vector<double>&inV, double dt, double &PL, double &PR);
...
{
public:
	afSABR(double alpha, double beta, double nu, double rho, double shift, double maturity, double F, int sizeX, int sizeT, double nd):shift_(shift),nd_(nd)
	{
		alpha_=alpha,beta_=beta,rho_=rho,nu_=nu,T_=maturity,f_=F,sizeX_=sizeX,sizeT_=sizeT;
	};
	~afSABR();
...
{
public:
	antonovSABR(double alpha, double beta, double nu, double rho, double maturity, double F, int sizeX, int sizeT,double nd):nd_(nd)
	{
		alpha_=alpha,beta_=beta,rho_=rho,nu_=nu,T_=maturity,f_=F,sizeX_=sizeX,sizeT_=sizeT;
	};
	~antonovSABR();
