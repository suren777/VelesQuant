...
class lBasket {
public:
  lBasket(Vdoub Spot, Vdoub Strike, Vdoub Maturities, Mdoub Forwards, Mdoub IV,
          Mdoub correlation);
  ~lBasket();
  ...
  double sim_basket_with_removal(double tT, double size, Vint &ishares,
                                 const Vdoub &barriers, const Vdoub &coupons,
                                 const Vdoub &initial_price, Vdoub &spot,
...
private:
  ...
  double interp_vol(std::vector<double> &T, std::vector<double> &F, double t1,
                    double t2) const;
  double interpolate(std::vector<double> &T, std::vector<double> &F, double t,
                     int asset) const;
