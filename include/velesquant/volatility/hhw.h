...
class HHW {
public:
  HHW(double s0, double v0, double r0, double kappa, double eta, double rho,
...
private:
  ...
  double b(double t) const { return c1_ - c2_ * exp(-t * c3_); };
  ...
  double ib(const double &t, const double &T) const;
  double intb(const double &T) const;
};
