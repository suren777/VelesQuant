...
public:
  ...
  lVol(std::vector<double> maturities, std::vector<double> forwards,
  ...
  std::vector<double> simulation(std::vector<double> times,
                                 std::vector<double> rands) const;

  void buildGrid(const std::vector<double> times);
  void buildGrid(double lastTime, int N = 100);

  void oneStepBackward(const int t, const std::vector<double> &inV,
                       std::vector<double> &outV);
  void oneStepBackwardDirichlet(const int t, const std::vector<double> &inV,
                                std::vector<double> &outV);

  double callPDE(double maturity, double strike, const int N = 100);
  double putPDE(double maturity, double strike, const int N = 100);
  double dntPDE(double maturity, double upperBarrier, double lowerBarrier,
                const int Nt = 100);

  void backwardOneStep(const int t, std::vector<double> &y,
                       const std::vector<double> &z);
  void forwardOneStep(const int t, const std::vector<double> &y,
                      std::vector<double> &z);
...
private:
  ...
  double premiumCALL(double strike, double forward, double maturity,
                     double vol) const;
  double interpolatedCall(const Sabr &nextModel, double strike, double forward,
                          double maturity) const;
  double interpolatedCall(const Sabr &preModel, const Sabr &nextModel,
  ...
  void getLocVol(double time, double preSpot, double preFwd, double &locvol,
                 double &forward) const;
  void getLocVolOLD(double time, double preSpot, double preFwd, double &locvol,
                    double &forward) const;
};
