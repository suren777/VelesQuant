...
private:
  ...
  double alpha(Vdoub aVec, int i, int type);
  double beta(Vdoub aVec, int i, int type);
  double gamma(Vdoub aVec, int i, int type);
  double delta(Vdoub aVec, int i, int type);

  void generateMeshes();

  double deltax(Vdoub aVec, int i);
  ...
  double extrapolate(double y1, double y2, double x1, double x2,
                           double x) {
  ...
  void fillA1(Vdoub &A1l, Vdoub &A1d, Vdoub &A1u);
  void fillA2(Vdoub A2l1, Vdoub &A2l2, Vdoub &A2d, Vdoub &A2u1,
                    Vdoub &A2u2);
  void fillA3(Vdoub &A3l, Vdoub &A3d, Vdoub &A3u, double t);
  ...
  void swap(Vdoub &Uin, Vdoub &Uout) { Uin.swap(Uout); };

  void LU3dec(Vdoub &a, Vdoub &b, Vdoub &c, Vdoub &l, Vdoub &d);
  void LU5dec(Vdoub &l, Vdoub &m, Vdoub &n, Vdoub &o, Vdoub &p,
                    /*Outputs:*/ Vdoub &b, Vdoub &c, Vdoub &d, Vdoub &e);
  void LU3decSparce(Vdoub &a, Vdoub &b, Vdoub &c, Vdoub &l, Vdoub &d,
                          int NS);

  void pentaSolve(Vdoub &E, Vdoub &A, Vdoub &D, Vdoub &C, Vdoub &F,
