...
/*!
        Heston Time Dependent model allows time dependency for mean-reverting
   parameter for volatility process as well as for volatility of volatility.
...
public:
  //! Class constructor
  sVolT(double spot, double var0, double kappa, double rho,
        std::vector<double> &time, std::vector<double> &theta,
        std::vector<double> &xit, int seed); ///
  sVolT(double spot, double var0, double kappa, double rho,
        std::vector<double> &time, std::vector<double> &theta, double xit,
        int seed);
  sVolT(double spot, double var0, double kappa, double rho,
        std::vector<double> &time, double theta, std::vector<double> &xit,
        int seed);
  ~sVolT() {};
  ...
  double getParameterXi(int i) const {
    if (i < nT_)
      return xit_[i];
    else
      throw("Memory Violation");
  };
  double getParameterTheta(int i) const {
    if (i < nT_)
      return thetat_[i];
    else
      throw("Memory Violation");
  ...
  std::vector<double> simulationHestonTdMax(std::vector<double> times,
                                            std::vector<double> forwards) const;
  std::vector<double> simulationHestonTdCliq(std::vector<double> times,
...
private:
  ...
  std::complex<double> hestonCFTd(std::complex<double> k,
                                  double maturity) const;
  void objFcnTd(int m, int n, double *x, double *fvec, int *iflag);
  //		double objFcnNM(vector<double>);
  double interpolateF(std::vector<double> T, std::vector<double> F,
                      double t) const;
  model_types model_;
};
