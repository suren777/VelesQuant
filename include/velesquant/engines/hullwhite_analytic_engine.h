...
private:
  ...
  static double cdf_normal(double x) {
    // Basic approximation or use boost if available in header.
    // For header-only, standard math or simple approx is better.
    // Ideally use boost::math::normal via boost header if dependency is okay.
    // Original code used boost.
