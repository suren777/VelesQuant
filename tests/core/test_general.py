...
def test_utilities():
    assert cdf_normal(0.0) == pytest.approx(0.5)
    assert pdf_normal(0.0) == pytest.approx(1.0 / math.sqrt(2 * math.pi))

    # Test implied vol
    # Call BS for price, then back it out
    spot, strike, r, d, vol, T = 100.0, 100.0, 0.05, 0.02, 0.2, 1.0
    price = black_scholes_call(spot, strike, r, d, vol, T)
    _ = implied_vol(
    ...
    price_black = black_formula_call(fwd, strike, vol, T)
    iv_black = implied_vol(T, fwd, strike, price_black)
...
def test_equity_models_basic():
    ...
    price = black_scholes_call(spot, strike, r, d, vol, T)
    assert price > 0
    vega = black_scholes_call_vega(spot, strike, r, d, vol, T)
    assert vega > 0
