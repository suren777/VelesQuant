#include <velesquant/local_vol/hhwPDE.h>
using namespace std;
#include <algorithm>
...
const double HHWPDE::deltax(Vdoub aVec, int i)
	{
	if ((i-1)<0) throw("Memory Violation\n");
	return aVec[i]-aVec[i-1];
	};

const double HHWPDE::alpha(Vdoub aVec,int i, int type)
	{
	double ret = 0;
	...
	if (type == -2)
		...
		ret = (b+a+a)/(b*(a+b));
	else throw("Wrong type in Alpha\n");
	return ret;
	};

const double HHWPDE::beta(Vdoub aVec,int i, int type)
	{
	double ret = 0;
	...
	if (type == -1)
		...
		ret = b/(a*(a+b));
	else throw("Wrong type in Beta\n");
	return ret;
	};

const double HHWPDE::gamma(Vdoub aVec,int i, int type)
	{
	double ret = 0;
	...
	if (type == 0)
		...
		ret = -b/(a*(a+b));
	else throw("Wrong type in Gamma\n");
	return ret;
	};

const double HHWPDE::delta(Vdoub aVec,int i, int type)
	{
	double ret = 0;
	double a = deltax(aVec,i+1);
	double b = deltax(aVec,i);
	if (type == -1)
		...
		ret = 2.0/(a*(a+b));
	else throw("Wrong type in Delta\n");
	return ret;
	};

const void HHWPDE::generateMeshes()
	{
	/*
	i = r index
	j = v index
...
void HHWPDE::aAbB(double a, Vdoub &Ain, double b, Vdoub &Bin,  Vdoub &Cout)
	...
	else throw("Error: dim(A) not equal to dim(B)\n");	
	};
	...
	for (int i = 0; i <= Mr_ ; i++)
		for (int j = 0; j <= Mv_ ; j++)
			for (int k = 0; k <=Ms_;k++)
				...
				rho12_*sigma1_*vM_[j]*sM_[k]*dudxdy(vM_,sM_,Uin,i,j,k,1)
				+rho13_*sigma2_*sM_[k]*sqrt(vM_[j])*dudxdy(rM_,sM_,Uin,i,j,k,3)
				+rho23_*sigma1_*sigma2_*sqrt(vM_[j])*dudxdy(rM_,vM_,Uin,i,j,k,2);	
	};
double HHWPDE::dudxdy(Vdoub &a, Vdoub &b,Vdoub &U, int i, int j, int k, int type)	
	{
	/*
	Types
	1: d^2/(dvds)
	...
	*/
	...
	if ((type == 3)&&((i==0)||(i==Mr_)||(j==Mv_)||(j==0)||(k==0)||(k==Ms_)))
		return 0;

	if ((type == 1)&&((j==Mv_)||(j==0)||(k==0)||(k==Ms_)))
		return 0;

	if ((type == 2)&&((i==0)||(i==Mr_)||(j==Mv_)||(j==0)||(k==0)))
		return 0;
	//---------------------------------------------------------//
	if (type == 1)
		for(int l = -1; l<=1;l++)
			for (int m=-1; m <= 1; m++)
				{
				if (((i+l)<0)||((i+l)>Mr_)) continue;
				else Sum+=beta(a,j,l)*beta(b,k,m)*fU(U, i, j+l, k+m);
				}
	else if (type == 2)
		for(int l = -1; l<=1;l++)
			for (int m=-1; m <= 1; m++)
				{
				if (k>Ms_) continue;
				else Sum+=beta(a,i,l)*beta(b,j,m)*fU(U, i+l, j+m, k);
				}
	else if (type == 3)
		for(int l = -1; l<=1;l++)
			for (int m=-1; m <= 1; m++)
...
const void HHWPDE::LU3dec(Vdoub &a, Vdoub &b, Vdoub &c, Vdoub &l, Vdoub &d)
	{
...
const void HHWPDE::LU3decSparce(Vdoub &c, Vdoub &a, Vdoub &b, Vdoub &l, Vdoub &d, int Ns)
	{
...
const void HHWPDE::LU5dec(Vdoub &l, Vdoub &m, Vdoub &n, Vdoub &o, Vdoub &p,
	/*Outputs:*/Vdoub &b, Vdoub &c, Vdoub &d, Vdoub &e)
	{

	d[0] = n[0]; e[0] = o[0];
	c[0] = m[0] / d[0]; d[1] = n[1] - c[0]*e[0]; e[1] = o[1] - c[0]*p[0];
	for (int i = 2 ; i < N_ ; i++ )
		{
		b[i-2] = e[i-2]/d[i-2];
		c[i-1] = (m[i-1]-b[i-2]*e[i-2])/d[i-1];
		d[i] = n[i] - b[i-2]*p[i-2]+c[i-1]*e[i-1];
		if (i < N_-1) e[i] = o[i] - c[i-1] * p[i-1];
		}
	};
	...
	for (int k = 0; k <= Ms_ ; k++)
		for (int j = 0; j <= Mv_ ; j++)
			...
			for (int i = 0; i <=Mr_; i++)
				...
				M=(sv*dS01[k]+rs*bS01[k] - 1.0/3.0*rM_[i]) * fU(Uin,i,j,k);
				if ( k == Ms_ ) 
					R = 0;
				else 
		...
		for (int k = 0; k <= Ms_ ; k++)
			for (int i = 0; i <=Mr_; i++)
				...
				if ( j == 0 ) 
					...
					M = (sigv*dVm1[j]+knuv*alpha(vM_,j,-1))*fU(Uin,i,j-1,k);
...
const void 
	HHWPDE::pentaSolve(Vdoub &E,Vdoub &A, Vdoub &D,Vdoub &C,Vdoub &F,Vdoub &B,Vdoub &X, int t)
	{
	static double xmult;
	int N;
	N = D.size();
...
const void HHWPDE::fillA1(Vdoub &A1l, Vdoub &A1d, Vdoub &A1u)
	{
	double sv, rs, L,M,R;
...
const void HHWPDE::fillA2(Vdoub A2l1, Vdoub &A2l2, Vdoub &A2d, Vdoub &A2u1, Vdoub &A2u2)
	{
	double L,M,R,sigv,knuv;
	int count;
	for (int j = 0; j <= Mv_ ; j++)
...
const void HHWPDE::fillA3(Vdoub &A3l, Vdoub &A3d, Vdoub &A3u, double t)
	{
	double s2 = .5 * sigma2_*sigma2_;
	int count = 0;
	for (int i = 0; i<=Mr_;i++)
		...
		for ( int j = 0 ; j < Mv_; j++)
			for (int k = 0; k < Ms_;k++)
				{
				count = i+Mv_*j+Mv_*Ms_*k;
				if (i > 0) A3l[count - Ms_*Mv_] = s2*dRm1[i]+abtr*bRm1[i];
				A3d[count]= s2*dR01[i] + abtr*bR01[i];
				if ( i < Mr_) A3u[count] = s2*dRp1[i] + abtr*bRp1[i];
				}
		}
	};
