...
lVol::lVol(std::vector<double> maturities, std::vector<double> forwards, std::vector<double> betas, 
...
static const double Wx[Mx] = { 0.010, 0.025, 0.050, 0.100, 0.150,
							   ...
							   1.525, 1.550, 1.575, 1.600, 1.650, 1.700, 
							   1.750, 1.800, 1.850, 1.900, 1.950, 2.000, 							   
							   2.250, 2.500, 3.000, 4.000, 5.000 };

std::vector<std::vector<double> > lVol::exportLV(const std::vector<double> times)
{
	buildGrid(times);	
	return gridLV_;
};
...
void lVol::oneStepBackward(const int t, const vector<double> &inV, vector<double> &outV)
{
	...
	for(int r=1; r<Mx-1; r++)
	{
		...
		double difu = 0.5 * gridLV_[t][r]*gridLV_[t][r] * gridFwd_[t][r]*gridFwd_[t][r];
		...
		Ru = gridFwd_[t+1][r+1];
		Rl = gridFwd_[t+1][r-1];
		difu =  0.5 * gridLV_[t+1][r]*gridLV_[t+1][r] * gridFwd_[t+1][r]*gridFwd_[t+1][r];
	...
	double difu = 0.5 * gridLV_[t][0]*gridLV_[t][0] * gridFwd_[t][0]*gridFwd_[t][0];
	...
	R1 = gridFwd_[t+1][1];
	R2 = gridFwd_[t+1][2];
	difu = 0.5 * gridLV_[t+1][0]*gridLV_[t+1][0] * gridFwd_[t+1][0]*gridFwd_[t+1][0];
	...
	R2 = gridFwd_[t][Mx-2];
	R1 = gridFwd_[t][Mx-1];
	difu = 0.5 * gridLV_[t][Mx-1]*gridLV_[t][Mx-1] * gridFwd_[t][Mx-1]*gridFwd_[t][Mx-1];
	...
	difu = 0.5 * gridLV_[t+1][Mx-1]*gridLV_[t+1][Mx-1] * gridFwd_[t+1][Mx-1]*gridFwd_[t+1][Mx-1];
...
void lVol::backwardOneStep(const int t, std::vector<double> &y, const std::vector<double> &z)
{
	...
	for(int x=1; x<Mx-1; x++)
	{
		...
		double var = 0.5 * delT * gridLV_[t][x]*gridLV_[t][x] * gridFwd_[t][x]*gridFwd_[t][x];
		...
		double var2 = 0.5 * delT * gridLV_[t+1][x]*gridLV_[t+1][x] * gridFwd_[t+1][x]*gridFwd_[t+1][x];
...
double lVol::dntPDE(double maturity, double upperBarrier, double lowerBarrier, const int Nt)
{
	gridT_.resize(Nt);
	gridFwd_.resize(Nt);
	gridLV_.resize(Nt);
...
void lVol::oneStepBackwardDirichlet(const int t, const vector<double> &inV, vector<double> &outV)
{
	int M = inV.size();
...
void lVol::forwardOneStep(const int t, const std::vector<double> &y, std::vector<double> &z)
{
	std::vector<double> a(Mx), b(Mx), c(Mx), d(Mx);
	...
	for(int x=1; x<Mx-1; x++)
	{
		...
		double delFm = 0.5*(gridFwd_[t][x+1]-gridFwd_[t][x-1]);
		double delFl = gridFwd_[t][x]-gridFwd_[t][x-1];
		double varu = 0.5 * delT * gridLV_[t][x+1]*gridLV_[t][x+1] * gridFwd_[t][x+1]*gridFwd_[t][x+1];
		double varm = 0.5 * delT * gridLV_[t][x]*gridLV_[t][x] * gridFwd_[t][x]*gridFwd_[t][x];
		double varl = 0.5 * delT * gridLV_[t][x-1]*gridLV_[t][x-1] * gridFwd_[t][x-1]*gridFwd_[t][x-1];
		...
		double delFm2 = 0.5*(gridFwd_[t-1][x+1]-gridFwd_[t-1][x-1]);
		double delFl2 = gridFwd_[t-1][x]-gridFwd_[t-1][x-1];
		double varu2 = 0.5 * delT * gridLV_[t-1][x+1]*gridLV_[t-1][x+1] * gridFwd_[t-1][x+1]*gridFwd_[t-1][x+1];
		double varm2 = 0.5 * delT * gridLV_[t-1][x]*gridLV_[t-1][x] * gridFwd_[t-1][x]*gridFwd_[t-1][x];
		double varl2 = 0.5 * delT * gridLV_[t-1][x-1]*gridLV_[t-1][x-1] * gridFwd_[t-1][x-1]*gridFwd_[t-1][x-1];
...
double lVol::premiumCALL(double strike, double forward, double maturity, double vol) const
{
...
double lVol::interpolatedCall(const sabr& nextModel, 
	double strike, double forward, double maturity) const
{
	...
	double strike, double forward, double maturity) const
{
	...
	double vars = preVar +max(0.0,nextVar-preVar)*(maturity-preMat)/(nextMat-preMat); //linear var
...
void lVol::getLocVol(double time, double preSpot, double preFwd, double &locvol, double &forward) const
{
	...
	{			
		...
		forward = spot_ +(afterForward-spot_)*time/firstMaturity;  //linear interpolation
		double currSpot = forward * fixRatio;
		double cVal = interpolatedCall(sabrModels_[0],currSpot,forward,time);
		double cLft = interpolatedCall(sabrModels_[0],currSpot*0.999,forward,time);
		double cRgt = interpolatedCall(sabrModels_[0],currSpot*1.001,forward,time);
		double dS2val = (cRgt+cLft-2.0*cVal)/(0.001*currSpot*0.001*currSpot);
		assert(dS2val>0.0);
		double cBigT = interpolatedCall(sabrModels_[0],currSpot,forward,time*1.001);
	...
	for(int i=1; i<N; i++)
	{
		...
		if(time > beforeMaturity && time < afterMaturity)  
		{
			...
			forward = beforeForward+(afterForward-beforeForward)*(time-beforeMaturity)/(afterMaturity-beforeMaturity);
			double currSpot = forward * fixRatio;
			double cVal = interpolatedCall(sabrModels_[i-1],sabrModels_[i],currSpot,forward,time);
			double cLft = interpolatedCall(sabrModels_[i-1],sabrModels_[i],currSpot*0.999,forward,time);
			double cRgt = interpolatedCall(sabrModels_[i-1],sabrModels_[i],currSpot*1.001,forward,time);
			double dS2val = (cRgt+cLft-2.0*cVal)/(0.001*currSpot*0.001*currSpot);
			assert(dS2val>0.0);
			double cBigT = interpolatedCall(sabrModels_[i-1],sabrModels_[i],currSpot,forward,time*1.001);
			double dTval = (cBigT-cVal)/(0.001*time);
			assert(dTval>0.0);
			locvol = std::sqrt(2.0*dTval/dS2val)/currSpot;
			//if(locvol != locvol) locvol = 0.0;	//if(locvol > 111.111) locvol = 111.111;
			return;
		}
...
double lVol::getForward(double time) const
{
	...
	{			
		double afterForward = sabrModels_[0].getForward();
		return spot_ +(afterForward-spot_)*time/firstMaturity;  //linear interpolation
	}
	int N = sabrModels_.size();
	double lastMaturity = sabrModels_[N-1].getMaturity();
	if(time > lastMaturity)
	...
	for(int i=1; i<N; i++)
	{
		...
		if(time > beforeMaturity && time <= afterMaturity)  
		{
			double beforeForward = sabrModels_[i-1].getForward();
			double afterForward = sabrModels_[i].getForward();
			return beforeForward+(afterForward-beforeForward)*(time-beforeMaturity)/(afterMaturity-beforeMaturity);
		}
	}
	return 0.0;
};
...
double lVol::getLocVol(double preTime, double preSpot, double preFwd) const
{
	...
	{			
		...
		double cLft = interpolatedCall(sabrModels_[0],preSpot*0.999,preFwd,preTime);
		double cRgt = interpolatedCall(sabrModels_[0],preSpot*1.001,preFwd,preTime);
		double dS2val = (cRgt+cLft-2.0*cVal)/(0.001*preSpot*0.001*preSpot);
		//assert(dS2val>0.0);
		dS2val = max(1.0E-20, dS2val);
		double cBigT = interpolatedCall(sabrModels_[0],preSpot,preFwd,preTime*1.001);
	...
	for(int i=1; i<N; i++)
	{
		...
		{
			double cVal = interpolatedCall(sabrModels_[i-1],sabrModels_[i],preSpot,preFwd,preTime);
			double cLft = interpolatedCall(sabrModels_[i-1],sabrModels_[i],preSpot*0.999,preFwd,preTime);
			double cRgt = interpolatedCall(sabrModels_[i-1],sabrModels_[i],preSpot*1.001,preFwd,preTime);
			double dS2val = (cRgt+cLft-2.0*cVal)/(0.001*preSpot*0.001*preSpot);
			//assert(dS2val>0.0);
			dS2val = max(1.0E-20, dS2val);
			double cBigT = interpolatedCall(sabrModels_[i-1],sabrModels_[i],preSpot,preFwd,preTime*1.001);
			double dTval = (cBigT-cVal)/(0.001*preTime);
			//assert(dTval>0.0);
			dTval = max(0.0, dTval);  //testing
			return std::sqrt(2.0*dTval/dS2val)/preSpot;
...
vector<double> lVol::simulation(vector<double> timesPath, vector<double> randsPath) const
{
	...
	for(int i=0; i<N; i++)
	{
		...
		double currentDrift = -0.5*currentVol*currentVol + currentVol*randsPath[i];  //Euler scheme
...
std::vector<double> lVol::simulation(std::vector<double> timesPath) const
{
	...
	{
		double currentTime = timesPath[i];
		if(preTime == 0.0)  preTime = 0.75 * currentTime; 
		...
		double currentDrift = currentVol*z -0.5*currentVol*currentVol; //Euler scheme
		double bumpLocVol = getLocVol(preTime, 1.001*preSpot, preFwd);
		double skewLocVol = (bumpLocVol-preLocVol)/(0.001*preSpot);
		currentDrift = currentDrift +0.5*(z*z-1.0)*preLocVol*(currentTime-preTime)*skewLocVol; //Milstein scheme
		double currentForward = getForward(currentTime); 
		double currentSpot = preSpot*exp(currentDrift) *(currentForward/preFwd);
		currentSpot = currentForward * min(max(0.01, currentSpot/currentForward), 50.0);
		spotsPath[i] = currentSpot;
		preSpot = currentSpot;
		preFwd = currentForward;
		preTime = currentTime;
...
//		double currentForward = getForward(currentTime); 
//		double currentSpot = currentForward * min(max(0.001, exp(pathDrift)), 500.0);
//		spotsPath[i] = currentSpot;
//		preSpot = currentSpot;
//		preFwd = currentForward;
//		preTime = currentTime;
...
std::vector<double> lVol::simulationTEST(std::vector<double> timesPath) const
{
	...
	for(int i=0; i<N; i++)
	{
		...
		getLocVol(currentTime,preSpot,preFwd,currentLocVol,currentForward);
		double currentVol = currentLocVol*sqrt(currentTime-preTime);
		double currentDrift = -0.5*currentVol*currentVol + currentVol*random_normal(); //Euler scheme
		...
		////if(skewLocVol < -9.9) skewLocVol = -9.9;	if(skewLocVol > 9.9) skewLocVol = 9.9;
		//double z = random_normal();
		//double currentDrift = -0.5*currentVol*currentVol + currentVol*z 
		//						+0.5*(z*z-1.0)*currentLocVol*(currentTime-preTime)*skewLocVol; //Milstein scheme
		driftPath = driftPath + currentDrift;
		currentSpot = currentForward * min(max(RFLO, std::exp(driftPath)), RTOP); //lognormal
		spotsPath[i] = currentSpot;
		preSpot = currentSpot;
		preFwd = currentForward;
		preTime = currentTime;
...
void lVol::getLocVolOLD(double time, double preSpot, double preFwd, double &locvol, double &forward) const
{
	double firstMaturity = sabrModels_[0].getMaturity();
	if(time <= firstMaturity)
	{			
		double nextForward = sabrModels_[0].getForward();
		forward = spot_ + time*(nextForward-spot_)/firstMaturity;  //linear interpolation
	...
	for(int i=1; i<N; i++)
	{
		...
		if( time > preMaturity && time <= nextMaturity)  
		{
			...
			forward = preForward + (time-preMaturity)*(nextForward-preForward)/(nextMaturity-preMaturity);
			double preLocvol = sabrModels_[i-1].localVolzabr(preSpot); 
			double nextSpot = nextForward * preSpot/preFwd;
			double nextLocvol = sabrModels_[i].localVolzabr(nextSpot); 
			locvol = preLocvol +(time-preMaturity)*(nextLocvol-preLocvol)/(nextMaturity-preMaturity); //linear locvol
			return;
		}
	}
};
