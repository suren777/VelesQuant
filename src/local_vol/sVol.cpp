...
vector<double> sVol::simulationHeston(vector<double> times, vector<double> forwards) const
	{
	int N=times.size();
	...
	for(int i=0; i<=int(times[N-1]/DT); i++)
		...
		S += S*sqrt(abs(V))*SDT*z1;  // This line (S) must put BEFORE line (V)!!!
		V += kappa_*(theta_-V)*DT +xi_*sqrt(abs(V))*SDT*z2 +0.25*xi_*xi_*DT*(z2*z2-1);  //Milstein scheme
...
vector<double> sVol::simulationHestonDO(vector<double> times, vector<double> forwards, double barrier) const
	{
	int N=times.size();
	...
	for(int i=0; i<=int(times[N-1]/DT); i++)
		...
		V += kappa_*(theta_-V)*DT +xi_*sqrt(abs(V))*SDT*z2 +0.25*xi_*xi_*DT*(z2*z2-1);  //Milstein scheme
		//V += kappa_*(theta_-V)*DT +xi_*sqrt(abs(V))*SDT*z2;  //Euler scheme
		if((i*DT <= times[T]) && ((i+1)*DT > times[T])&&(S<=barrier)) flag = 0.0;
		}	
		}
...
void sVol::update_seed(int seed)
	{
	set_seed(seed);
	};
...
sVol::sVol(double spot, double var0, double kappa, double theta, double xi, double rho, int seed)
	: spot_(spot), var0_(var0), kappa_(kappa), theta_(theta), xi_(xi), rho_(rho)
	{ 
	if (seed == 0)
...
double sVol::hestonPrice(double maturity, double forward, double strike, string optType) const
	{
	//if ((forward/strike>1e5)||(strike==0.0)) return forward-strike;
	if ((strike==0.0)) return forward;
	else
		{
	boost::function<double (double)> fcn1, fcn2;


	fcn1 = boost::bind(&sVol::hestonIntegrand, this, _1, maturity, forward, strike, 0);
	fcn2 = boost::bind(&sVol::hestonIntegrand, this, _1, maturity, forward, strike, 1);
	GaussLaguerreIntegration gLegInt(32);
	double integr1 = gLegInt(fcn1);
	double integr2 = gLegInt(fcn2);
	double callPrice = forward*(0.5+integr2/PI)-strike*(0.5+integr1/PI);
	
	if (callPrice<=0.0) return 0;
...
double sVol::hestonIntegrand(complex<double> k, double maturity, double forward, double strike, int choice) const
	{
	...
	complex<double> F1 = kappa_*(r2*maturity-2.0/(xi_*xi_)*log((1.0-g*Y)/(1.0-g)));
...
double sVol::hestonPriceCF(double maturity, double forward, double strike, string optType) const
	{
	if (strike==0.0) return forward;
...
double sVol::intCFfun(double u, double ki, double X, double maturity)const
	{
	complex<double> ret = exp(complex<double>(0.0,1.0)*u*X)*hestonCF(complex<double>(u,ki), maturity);
	return ret.real()/(u*u+ki*ki);
	}
void sVol::calibrator(vector<double> maturitys, vector<double> forwards, vector<double> strikes, 
	...
	boost::function<void (sVol*, int,int,double*,double*,int*)> objheston = &sVol::objFcn;
	lmfcn fcnheston = boost::bind(objheston, this, _1,_2,_3,_4,_5);
	lmdif(m, n, x, fvec, ftol, xtol, gtol, maxfev, epsfcn, diag, mode, factor,
		nprint, &info, &nfev, fjac, ldfjac, ipvt, qtf, wa1, wa2, wa3, wa4, fcnheston);
...
void sVol::IVcalibrator(vector<double> maturitys, vector<double> forwards, vector<double> strikes, 
	std::vector<double> marketQuotes, string quoteType) 
	{
	int m = strikes.size();		//no. of observations
	weights_.resize(m);
	...
	marketQuotes_.resize(m);
	marketQuotes_ = marketQuotes;
	if(quoteType != "impliedVol") calibrator(maturitys, forwards, strikes, marketQuotes,quoteType);
	else{
	int n = 5;					//no. of HESTON model paremeters 
	double* x = new double[n];	//initial estimate of parameters vector
	x[0] = getParameterVar0();
	...
	boost::function<void (sVol*, int,int,double*,double*,int*)> objheston = &sVol::objFcnIV;
	lmfcn fcnheston = boost::bind(objheston, this, _1,_2,_3,_4,_5);
	lmdif(m, n, x, fvec, ftol, xtol, gtol, maxfev, epsfcn, diag, mode, factor,
		nprint, &info, &nfev, fjac, ldfjac, ipvt, qtf, wa1, wa2, wa3, wa4, fcnheston);
	//*   info is an integer output variable. if the user has terminated execution, info is set to the (negative)
...
void sVol::FXcalibrator(vector<double> maturitys, vector<double> forwards, vector<double> strikes, 
	std::vector<double> marketQuotes, string quoteType) 
	{
	int m = strikes.size();		//no. of observations
	//weights_.resize(m);
	...
	boost::function<void (sVol*, int,int,double*,double*,int*)> objheston = &sVol::objFcnFX;
	lmfcn fcnheston = boost::bind(objheston, this, _1,_2,_3,_4,_5);
	lmdif(m, n, x, fvec, ftol, xtol, gtol, maxfev, epsfcn, diag, mode, factor,
		nprint, &info, &nfev, fjac, ldfjac, ipvt, qtf, wa1, wa2, wa3, wa4, fcnheston);
	//*   info is an integer output variable. if the user has terminated execution, info is set to the (negative)
...
//	setParameterRho(x[4]);
//	for(int i=0; i<m; i++)
//		fvec[i] = hestonPrice(maturitys_[i], forwards_[i], strikes_[i], "call") - marketQuotes_[i];
//	};
...
void sVol::objFcn(int m, int n, double* x, double* fvec, int* iflag)
	...
	setParameterRho(x[4]);
	for(int i=0; i<m; i++)
	//	fvec[i] = (hestonPrice(maturitys_[i], forwards_[i], strikes_[i], "call") - marketQuotes_[i])*weights_[i];
	  fvec[i] = (hestonPrice(maturitys_[i], forwards_[i], strikes_[i], "call") - marketQuotes_[i]);
	// fvec[i] = (hestonPriceCF(maturitys_[i], forwards_[i], strikes_[i], "call") - marketQuotes_[i]);
	};
void sVol::objFcnIV(int m, int n, double* x, double* fvec, int* iflag)
	{
	setParameterVar0(x[0]);
	setParameterKappa(x[1]);
	setParameterTheta(x[2]);
	...
	setParameterRho(x[4]);
	double A = 0.0;
	if (x[1]*x[2]*2.0<x[3]*x[3]) A = 1000000;
	
	for(int i=0; i<m; i++)
		fvec[i] = (implied_vol(maturitys_[i], forwards_[i], strikes_[i],hestonPrice(maturitys_[i], forwards_[i], strikes_[i], "call")) - marketQuotes_[i])+A;
	};
void sVol::objFcnFX(int m, int n, double* x, double* fvec, int* iflag)
	{
	setParameterVar0(x[0]);
	setParameterTheta(x[1]);
	setParameterXi(x[2]);
	setParameterRho(x[3]);
	double A = 0.0;
	if (kappa_*x[1]*2.0<x[2]*x[2]) A = 10000;
	for(int i=0; i<m; i++)
		fvec[i] = 1.0-(hestonPrice(maturitys_[i], forwards_[i], strikes_[i], "call")+A)/marketQuotes_[i];
	};


// NOT USED!!!
double sVol::hestonIntegrandNET(complex<double> k, double maturity, double forward, double strike) const
	{
	complex<double> i(0.0, 1.0);
	...
	complex<double> F1 = kappa_*theta_*(r1*maturity+2.0/(xi_*xi_)*log((1.0-g)/(1.0-g*Y)));
	complex<double> F2 = r1*(1.0-Y)/(1.0-g*Y);
	complex<double> integrand = exp(-i*k*log(forward/strike)+F1+F2*var0_)/(k*k-i*k);
	if(real(integrand) != real(integrand)) 
		return 0.0;
	else
		return real(integrand);
	}
double sVol::hestonPriceNET(double maturity, double forward, double strike, string optType) const
	{
	const double ki = 0.5;
...
vector<double> sVol::simulationHestonMax(vector<double> times, vector<double> forwards) const
	{
	int N=times.size();
	vector<double> pathF(N);
	int T = 0;
...
vector<double> sVol::simulationHestonCliq(vector<double> times, vector<double> forwards, 
		double gcap, double gfloor, double lcap, double lfloor, double alpha) const
{
	int N=times.size();
	vector<double> pathF(N);
	int T = 0;
...
vector<double> sVol::simulationHestonDNT(vector<double> times, vector<double> forwards, 
...
double sVol::simulationHestonDNTdt(vector<double> times, vector<double> forwards, 
		double UP, double DOWN, double dt) const
{
	...
	for(int i=0; i<=Nstep; i++)
	{
		...
		V += kappa_*(theta_-V)*dt +xi_*sqrt(abs(V))*sdt*z2 +0.25*xi_*xi_*dt*(z2*z2-1);  //Milstein scheme
		S1 = interpolateF(times,forwards,i*dt)*S/spot_*pr;
		if (!((S1<=iUP)&&(S1>=iDOWN))) return 0.0;
	}
	return 1.0;
}
int sVol::simulationHestonDNTdtS(vector<double> times, vector<double> forwards, double maturity, 
		double UP, double DOWN, double dt) const
{
	...
	for(int i=0; i<=Nstep; i++)
	{
		...
		V += kappa_*(theta_-V)*dt +xi_*sqrt(abs(V))*sdt*z2 +0.25*xi_*xi_*dt*(z2*z2-1);  //Milstein scheme
		S1 = interpolateF(times,forwards,i*dt)*S/spot_*pr;
		if (!((S1<=iUP)&&(S1>=iDOWN))) return 0;
	}
	return 1;
}
int sVol::simulationHestonDNTdtE(vector<double> times, vector<double> forwards, double maturity, 
		double UP, double DOWN, double dt) const
{
	int Nstep = maturity/dt;
	int pr = 1e6;
...
vector<double> sVol::simulationHestonUpnOut(vector<double> times, vector<double> forwards, 
		double BARRIER) const
{
	
	int N=times.size();
	vector<double> pathF(N,0.0);
	if (BARRIER <= spot_) return pathF;
	int T = 0;
	double V=var0_, S=spot_, rho2 = sqrt(1-rho_*rho_);
	double S1;
	for(int i=0; i<=int(times[N-1]/DT); i++)
...
vector<double> sVol::simulationHestonDownnOut(vector<double> times, vector<double> forwards, 
		double BARRIER) const
{
