...
sVolT::sVolT(double spot, double var0, double kappa,double rho, Vdoub  &time, Vdoub &theta, Vdoub &xit, int seed)
...
sVolT::sVolT(double spot, double var0, double kappa,double rho, Vdoub  &time, Vdoub &theta,  double xit, int seed)
	:spot_(spot), var0_(var0), kappa_(kappa), rho_(rho)
{
...
sVolT::sVolT(double spot, double var0, double kappa,double rho, Vdoub  &time, double theta, Vdoub &xit, int seed)
	:spot_(spot), var0_(var0), kappa_(kappa), rho_(rho)
{
...
double sVolT::hestonIntegrandTd(Cdoub k, double maturity, double forward, double strike, int choice) const
{
	Cdoub i(0.0, 1.0);
	Cdoub a,b;
	...
	if (maturity > t_[nT_-1] ) throw ("Maturity is out of range");
	...
	for ( int j=0; j<nT_ ; j++)
	{
		...
		Y	= exp(-d*dT);
		F1 += kappa_*theta/xi2*((b-d)*dT-2.0*log((1.0-gn*Y)/(1.0-gn)));
...
double sVolT::hestonPriceTd(double maturity, double forward, double strike, string optType) const
{
	boost::function<double (double)> fcn1, fcn2;
	fcn1 = boost::bind(&sVolT::hestonIntegrandTd, this, _1, maturity, forward, strike, 0);
	fcn2 = boost::bind(&sVolT::hestonIntegrandTd, this, _1, maturity, forward, strike, 1);
...
double sVolT::hestonPriceTdCF(double maturity, double forward, double strike, string optType) const
{
	if (strike==0.0) return forward;
	double X = log(forward/strike);
	double ki = -0.5;
	boost::function<double (double)> fcn1;
	fcn1 = boost::bind(&sVolT::intCFfun, this, _1, ki, X, maturity);
...
void sVolT::objFcnTd(int m, int n, double* x, double* fvec, int* iflag)
{
	...
	{
	case 0:
		for( int i = 0; i < nT_; i++) setParameterTheta(x[3],i);
		for( int i = 0; i < nT_; i++) setParameterXi(x[4+i],i);			
		break;
	case 1:
		for( int i = 0; i < nT_; i++) setParameterTheta(x[3+i],i);	
		for( int i = 0; i < nT_; i++) setParameterXi(x[3+nT_], i);
		break;
	case 2:
		for( int i = 0; i < nT_; i++) setParameterTheta(x[3+i],i);
		for( int i = 0; i < nT_; i++) setParameterXi(x[3+i+nT_],i);
	}
	for(int i=0; i<m; i++)
		fvec[i] = (1.0 - hestonPriceTdCF(maturities_[i], forwards_[i], strikes_[i], "call") / marketQuotes_[i])*100;
};
						 ...
						 Vdoub marketQuotes, string quoteType) 
{
	...
	if (n > int(maturities.size())) throw("Need more data points!!!");
	...
	case 0:
		...
		for( int i = 0; i < nT_; i++) x[4+i]=getParameterXi(i);			
		break;
	case 1:
		for( int i = 0; i < nT_; i++) x[3+i]=getParameterTheta(i);	
		x[3+nT_] = getParameterXi(0);
		break;
	case 2:
		for( int i = 0; i < nT_; i++) x[3+i]=getParameterTheta(i);
		for( int i = 0; i <  nT_; i++) x[3+i+nT_]=getParameterXi(i);
	}
	...
	boost::function<void (sVolT*, int,int,double*,double*,int*)> objheston = &sVolT::objFcnTd;
...
Vdoub sVolT::simulationHestonTd(Vdoub times, Vdoub forwards) const
{
	...
	double V=var0_, S=spot_, rho2 = sqrt(1-rho_*rho_),xin=xit_[0],theta = thetat_[0],accT=0, x = 0;
	...
	{	
		...
		V += kappa_*(theta-V)*DT +xin*sqrt(abs(V))*SDT*z2;  //Euler scheme
...
Vdoub sVolT::simulationHestonTdCliq(Vdoub times, Vdoub forwards,
...
vector<double> sVolT::simulationHestonTdCliq(vector<double> times, vector<double> forwards, 
											 double gcap, double gfloor, 
											 double lcap, double lfloor, 
											 double alpha, int tDay, Termstructure *mycurve) const
{
	int T=0, N=times.size(),j=0;
	Vdoub pathF(N);
	...
	{	
		...
		if((i*DT<=times[T]) && ((i+1)*DT>times[T]))
		{
			pathF[T] = std::min(std::max(cRet,gfloor),gcap)*exp((r-q)*(times[T+1]-times[T]));
			S01 = S;
			T++;
...
void sVolT::update_seed(int seed)
{
	set_seed(seed);
};
...
{
double aux = 0.0;
aux = hestonPrice1(maturities_[i], forwards_[i], strikes_[i], "call") - marketQuotes_[i];
sum += aux*aux;
}
return sum;
};
