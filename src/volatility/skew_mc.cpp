...
skewMC::skewMC(std::vector<double> maturities, std::vector<double> forwards,
...
skewMC::skewMC(std::vector<double> maturities, std::vector<double> forwards,
  ...
  sabrModels_.resize(N);
  for (int i = 0; i < N; i++) {
    sabrModels_[i] = Sabr(maturities[i], forwards[i], betas[i], alphas[i],
                          nus[i], rhos[i], shifts[i]);
  }
};

std::vector<double> skewMC::simulation(std::vector<double> timesPath,
                                       double spot, double kappa) {
...
double skewMC::simulatedSpot(double time, double spot, double corrRN) {
  ...
  if (time < firstMaturity) {
    ...
    // double bBridge = sqrt(time)*random_normal() - time/firstMaturity *
    // sqrt(firstMaturity)*random_normal(); return bBridge +
    // (spot+time*(nextSpot-spot)/firstMaturity);
  }
  ...
  for (int i = 1; i < N; i++) {
    ...
    if (time >= preMaturity && time < nextMaturity) {
      ...
      return preSpot + (time - preMaturity) * (nextSpot - preSpot) /
                           (nextMaturity - preMaturity);
    }
  }
  QL_FAIL("Time is out of range in skewMC::simulatedSpot");
  ...
  // return bBridge +
  // (preSpot+(time-preMaturity)*(nextSpot-preSpot)/(nextMaturity-preMaturity));
}

vector<double> skewMC::autoCorrRandoms(vector<double> times,
                                       double kappa) const {
...
vector<vector<double>> skewMC::autoCorrMatrix(vector<double> times,
                                              double kappa) const {
  int N = times.size();
  ...
  for (int i = 0; i < N; i++) {
    ...
    for (int j = 0; j < i; j++) {
      if (kappa < 1.0E-10)
        ...
        corrMatrix[i][j] = std::sqrt((std::exp(2 * kappa * times[j]) - 1) /
                                     (std::exp(2 * kappa * times[i]) - 1));
...
skewMC::cholesky(vector<vector<double>> corrMatrix) const {
...
/*
vector<vector<double> > skewMC::cholesky(vector<vector<double> > corrMatrix)
const
{
    int N=corrMatrix.size();
    ...
    for(int i=0; i<N; i++)
        for(int j=0; j<i; j++)
            if(corrMatrix[i][j] != corrMatrix[j][i]) throw("input matrix is not
symmetric"); vector<vector<double> > matrixCholesky; matrixCholesky.resize(N);
        for(int i=0; i<N; i++)
        {
                matrixCholesky[i].resize(N);
                for(int j=0; j<N; j++)
...
root of sum if positive, else zero. matrixCholesky[i][i] =
std::sqrt(std::max(sum, 0.0));
            }
                        else
                // With positive semi-definite matrices is possible to have
matrixCholesky[i][i]==0.0 In this case sum happens to be zero as well
                matrixCholesky[j][i] = (sum==0.0 ? 0.0 :
sum/matrixCholesky[i][i]);
        }
        return matrixCholesky;
};
*/
