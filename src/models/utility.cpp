...
double annuity(double Expiry, double Tenor, double Freq,
               const vector<double> &DF, const vector<double> &T) {
  double ti, DFi;
  double annuity = 0.0;
...
double fwdSR(double Expiry, double Tenor, double Freq, const vector<double> &DF,
...
vector<vector<double>> cholesky(const vector<vector<double>> &corrMatrix) {
...
void lAxy(const vector<vector<double>> &lowerTriangular,
          const vector<double> &input, vector<double> &result) {
  int n = input.size();
  for (int i = 0; i < n; i++) {
    result[i] = 0;
    for (int j = 0; j <= i; j++)
...
vector<double> DFtoR(const vector<double> &DF, const vector<double> &T, int n) {
  int m = DF.size();
  vector<double> R0s(m);
  ...
  for (int i = 0; i < m; i++) {
    if (n == 0) {
      ...
      R0s[i] = n * (pow(1.0 / DF[i], 1 / (n * T[i])) - 1.0);
  }
...
vector<double> DFtoDiv(const vector<double> &DF, const vector<double> &dfT,
                       const vector<double> &Fwd, const vector<double> &fwdT,
                       double Spot, int n) {
  int m = Fwd.size();
...
vector<double> DFtoFwd(const vector<double> &DF, const vector<double> &T,
                       double delta, int n) {
  int m = DF.size();
...
vector<double> MCimpVol(double freq, const vector<double> &theTimes,
                        const vector<vector<double>> &thelogReturns) {
  ...
  double sum =
      accumulate(thelogReturns[0].begin(), thelogReturns[0].end(), 0.0);
  double mean = sum / n;
  double sq_sum =
      inner_product(thelogReturns[0].begin(), thelogReturns[0].end(),
