...
cms_spread::cms_spread(double expirySR1, double tenorSR1, double forwardSR1, double annuitySR1, 
	...
	: corr_(corr)
{
	sabrCMS1_ = new cms(expirySR1, tenorSR1, forwardSR1, annuitySR1, payCMS1, discountCMS1, 
						beta1, strikes1, marketQuotes1, quoteType1);		
	sabrCMS2_ = new cms(expirySR2, tenorSR2, forwardSR2, annuitySR2, payCMS2, discountCMS2, 
						beta2, strikes2, marketQuotes2, quoteType2);		
};

// Added function by James Carey on 19-11-2015 to allow for alternative convexity adjustment calculation
cms_spread::cms_spread(double expirySR1, double tenorSR1, double forwardSR1, double freqSR1, double freqCMS1, 
		double payCMS1, double discountCMS1, 
		double beta1, CellMatrix& strikes1, CellMatrix& marketQuotes1, string quoteType1,
		...
		: corr_(corr)
{
	sabrCMS1_ = new cms(expirySR1, tenorSR1, forwardSR1, freqSR1, freqCMS1, payCMS1, discountCMS1, 
						beta1, strikes1, marketQuotes1, quoteType1);		
	sabrCMS2_ = new cms(expirySR2, tenorSR2, forwardSR2, freqSR2, freqCMS2, payCMS2, discountCMS2, 
						beta2, strikes2, marketQuotes2, quoteType2);
};
...
double cms_spread::margrabe() const
{
	...
	double sigmaT = sqrt(Maturity) * sqrt(atmVol1*atmVol1 +atmVol2*atmVol2 -2*corr_*atmVol1*atmVol2);
...
double cms_spread::spreadOption(double K, double a, double b) const //p151
{
	...
	fcn = boost::bind(&cms_spread::intFun, this, _1, pn1,K,alpha,sigma1,beta,sigma2,rho);
...
double cms_spread::intFun(double epsilon, int pn1, double K, double alpha, double sigma1, 
		double beta, double sigma2, double rho) const
{
	...
	return pdf_normal(epsilon)*(int1 + beta*exp(0.5*sigma2*sigma2*(1-rho*rho))*int2);
};

double cms_spread::lstar(double epsilon, double K, double alpha, double sigma1, 
		double beta, double sigma2, double rho) const
{
	double pn = (K-alpha*exp(sigma1*epsilon))/(beta*exp(rho*sigma2*epsilon));
	double ls = -1.0E+20;
	if(pn > 0.0)
