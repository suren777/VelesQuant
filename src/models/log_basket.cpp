...
lBasket::lBasket(Vdoub Spot, Vdoub Strike, Vdoub Maturities, Mdoub Forwards,
                 Mdoub IV, Mdoub correlation) {
...
void lBasket::update_seed(int seed) { set_seed(seed); }
...
Mdoub lBasket::simulate_basket(Vdoub schedule) {
  ...
  for (int k = 0; k < N; k++) {
    ...
    for (int i = 0; i < Nassets_; i++) {
      ...
      pathF[i][k] =
          S[i] * interpolate(maturity_, forward_[i], t1 + dT, i) / spot_[i];
    }
    t1 += dT;
  }
...
double lBasket::interp_vol(vector<double> &T, vector<double> &F, double t1,
                           double t2) const {
...
double lBasket::interpolate(vector<double> &T, vector<double> &F, double t,
                            int asset) const {
  int i, N = T.size();
  double spot = 0.0;
  if (asset == -1)
...
double lBasket::sim_basket_with_removal(double tT, double size, Vint &ishares,
                                        const Vdoub &barriers,
                                        const Vdoub &coupons,
                                        const Vdoub &initial_price, Vdoub &S,
                                        int Nsteps, int called) {
  ...
  for (int k = 0; k < Nsteps; k++) {
    ...
    for (int i = 0; i < Nassets_; i++) {
      if (ishares[i] == 1) {
        ...
        step[i] = S[i] / initial_price[i] *
                  interpolate(maturity_, forward_[i], t1 + dT, i) / spot_[i];
  ...
  if (min < barriers[1])
    ...
    coupon = coupons[0];
    if (called == 1)
      for (int i = 0; i < int(ishares.size()); ishares[i++] = 0)
        ;
  }
  ishares[takeout] = 0;
  return coupon;
...
void lBasket::get_spots(Vdoub &S) {
  for (int i = 0; i < int(spot_.size()); i++)
    S[i] = spot_[i];
};
} // namespace velesquant