...
void ShortRate1FPDE::pricingCouponBondt(double t0, double Tn, double Coupon, double PayFrequency,vector<double> &f)
{
...
double ShortRate1FPDE::pricingCouponBond(double Expiry, double Tenor, double Coupon, double PayFrequency)
{
...
double ShortRate1FPDE::pricingCBO(double Expiry, double Tenor, double Coupon, double Strike, double PayFrequency, const std::string &type)
{
...
double ShortRate1FPDE::pricingZBO(double Expiry, double Maturity, double Strike, const std::string &type)
{
...
double ShortRate1FPDE::pricingSwap(double Expiry, double Tenor, double Strike, double PayFrequency)
{
	vector<double> payoff(MR,-1);
...
double ShortRate1FPDE::pricingCallableSwap(double Expiry, double Tenor, vector<double> Exercises, double Coupon, double Strike, double PayFrequency, const std::string &type)
{
	...
	for(int e=Ne-2; e>=0; e--)
	{
		...
		{
#pragma omp parallel for
			for(int r=0; r<MR; r++)	call_value[r] =(type=="Call")? max(0.0, (payoff[r]+1.0)-Strike): max(0.0, -(payoff[r]+1.0)+Strike);// payoff	
		}
		else
		{
...
#pragma omp parallel for
	...
	if (Expiry>0) discountBack(0,Expiry,call_value);
	return	call_value[iR0_];
};

double ShortRate1FPDE::pricingSwaption(double Expiry, double Tenor, double Strike, double PayFrequency)
{
	vector<double> payoff(MR,-1.0);
	pricingCouponBondt(Expiry,Expiry+Tenor, -Strike, PayFrequency, payoff);
...
double ShortRate1FPDE::pricingBermudan(double Expiry, double Tenor, vector<double> Exercises, double Strike, double PayFrequency)
{
...
vector<double> ShortRate1FPDE::calculateDFs(vector<double>& timePoints)
{
	...
	for(int r=1; r<MR-1; r++)
		if(gridR_[r] >= R0_)
		{ 
			inV[r] = 2.0/(gridR_[r+1]-gridR_[r-1]); // Dirac delta as initial distribution 
			break;
		}
		for(int i=0; i<n; i++)
		{
...
void ShortRate1FPDE::calibrator(vector<double> timeDFs, vector<double> DFs, vector<defSwap> swapQuotes)
{
	DFs_ = DFs;
	timeDFs_ = timeDFs;
	...
	timeThetas_[0]=dt;
	for (double i = timeThetas_[0] + dt ; i <= timeDFs_[timeDFs_.size()-1]; i +=dt)	timeThetas_.push_back(i);
	thetas_.resize(timeThetas_.size());
	for (int i = 0; i< timeThetas_.size();i++) thetas_[i]=.001;
	DFinterp_.resize(timeThetas_.size());
	for (int i = 0 ; i < timeThetas_.size(); i++ ) DFinterp_[i] = getDFinterp(timeThetas_[i]);
	...
	for(int i=0; i<ns; i++)	x[i] = sigmas_[i];		// sigmas initial value
	calibrateKappa();
	...
	for (int i = 0; i<m;i++) quoteSwap_[i].Value = swaptionATM(quoteSwap_[i].Expiry, quoteSwap_[i].Tenor, quoteSwap_[i].VolATM);
	...
	boost::function<void (ShortRate1FPDE*, int,int,double*,double*,int*)> obj = &ShortRate1FPDE::objFcnCalibrator;
...
double ShortRate1FPDE::pen_fun(double*x, int n)
{
	double penalty = 0.0;
	double lambda = 1e6;
	//for(int i=0; i<n-2; i++)penalty+=((x[i]<=0)||(x[i]>=1))?lambda*pow(x[i],2):0;
	penalty+=(beta_>=alpha_)?lambda*beta_*beta_:0;
	return penalty;
}

void ShortRate1FPDE::objFcnCalibrator(int m, int n, double* x, double* fvec, int* iflag)
{
	int ns = sigmas_.size();
	for(int i=0; i<ns; i++)
		sigmas_[i] = fabs(x[i]);	
...
#pragma omp parallel for
		for(int i=0; i<MR; i++)	lastV[i] = inV[i];// remember the starting density
		for(int i=0; i<n; i++)
		{
			int sT = 0;
			if(i > 0)	sT = iTs[i-1] - 1;
...
#pragma omp parallel for
				...
				for(int t=sT; t<iTs[i]-1; t++)	oneStepForward(t, inV);
				df = trapezoidal(inV);
				thetas_[i] *= (1 + 0.001*(DFinterp_[i]-df)/(dfUP-df));	// Newton iteration
...
{
	return theta-kappa_*gridR_[j]-p*pow(alpha_+beta_*gridR_[j],2*gamma_-1);
}
...
{
	return p*pow(alpha_+beta_*gridR_[j],2*gamma_-2)+kappa_-gridR_[j];
}
...
double ShortRate1FPDE::objKappa(double a, const vector<double> &ivr, const vector<double> &br, const vector<int> &ind)
{
	int N = ivr.size();
	double sum = 0.0;
	for (int i = 1; i< N; i++)
...
vector<double> ShortRate1FPDE::SwaptionDiagnostic(double Expiry, double Tenor, double Strike, double PayFrequency)
{
...
#pragma omp parallel for
	...
	for(int t=iExpiry-1; t>=0; t--)	oneStepBackward(t, payoff);
	return payoff;
};
...
vector<double> ShortRate1FPDE::RiskNeutralDensity(double T1, double T2)
{
	...
	for(int r=1; r<MR-1; r++)
		...
		for(int t=iTs[0]; t<iTs[1]-1; t++) oneStepForward(t, inV);
		return inV;
};

double ShortRate1FPDE::getSwapRate(double Expiry, double Tenor, double PayFrequency) 
{
...
double ShortRate1FPDE::getImpVolATM(double Expiry, double Tenor, double PayFrequency)
{
