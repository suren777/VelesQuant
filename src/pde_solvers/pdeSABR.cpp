...
{
	zmax_=zmin_=-3*sqrt(T_);zmax_*=-1;
	if (beta_<1)
	{
		double aux = -pow(f_+shift_,1-beta_)*nu_/alpha_;
		double zb = -1/nu_*log((sqrt(1-rho_*rho_+(rho_+aux)*(rho_+aux))-rho_-nu_*aux/alpha_)/(1-rho_));
...
{
	vector<double> y;
	for(int i = 0; i < z.size()-1; i ++) y.push_back(Y(z[i]));
	return y;
}
...
{
	vector<double> y;
	for(int i = 0; i < z.size()-1; i ++) y.push_back(F(z[i]));
	return y;
}
...
{
	vector<double> aux;
	for(int i = 0; i < y.size()-1; i ++) y.push_back(C(y[i],f[i]));
	return aux;
}
...
double pdeSABR::L(double F)
{
	return pow(F+shift_,beta_);
}
double pdeSABR::Lm(double F)
{
...
double pdeSABR::Mshifted(double F, double t)
{
	...
	double nex = 0.5*eps_*eps_*alpha_*alpha_*(1+2*eps_*rho_*nu_*z+eps_*eps_*nu_*nu_*z*z);
	return nex*ex*l*l;
};

double pdeSABR::Mnormal(double F, double t)
{
	return 0.5*eps_*eps_*(alpha_*alpha_+2*rho_*nu_*alpha_*(F-f_)+nu_*nu_*(F-f_)*(F-f_));
}
...
double pdeSABR::volNormal(double K)
{
	...
	return eps_*alpha_*zeta/x*(1+(.25*rho_*nu_*alpha_+1/24.0*(2-3*rho_*rho_)*nu_*nu_)*eps_*eps_*T_);
};
...
double pdeSABR::volShifted(double K)
{
	...
	double p3 = 0.25*rho_*nu_*alpha_*fbk/(f_-K)+(2-3*rho_*rho_)/24*nu_*nu_;
...
void pdeSABR::calibrator(vector<volQuote> &quotes)
{
	...
	boost::function<void (pdeSABR*, int,int,double*,double*,int*)> obj = &pdeSABR::objFcnCalibration;
...
void pdeSABR::objFcnCalibration(int m, int n, double* x, double* fvec, int* iflag)
{
	alpha_ = x[0];
	rho_ = sin(x[1]);
	nu_ = x[2];
...
#pragma omp parallel for 
	for(int r=1; r<=sizeX_; r++)
	{
		if (r==1) 
		{
			...
			d[r-1] = inV[r]+ndt*(-2*Mf((r-0)*h_,t0)*inV[r-0]+Mf((r+1)*h_,t0)*inV[r+1]);
		}
		else if (r==sizeX_-1) 
		{
			u[r-1]=0;
			...
			c[r-1] = 3*ndt*Mf((r)*h_,t1)+1.0;
			d[r-1] = inV[r]+ndt*(Mf((r-1)*h_,t0)*inV[r-1]-3*Mf((r-0)*h_,t0)*inV[r-0]);
		}
		else
		{
			d[r-1] = inV[r]+ndt*(Mf((r-1)*h_,t0)*inV[r-1]-2*Mf((r-0)*h_,t0)*inV[r-0]+Mf((r+1)*h_,t0)*inV[r+1]);
			l[r-1] = -ndt*Mf((r-1)*h_,t1);
	...
	inV[sizeX_-1]=inV[sizeX_-1]-dT_/h_*(Mf(h_*sizeX_,t1)*inV[sizeX_-2]+Mf(h_*sizeX_,t0)*aux[sizeX_-1]);
#pragma omp parallel for 
	for(int i = 0; i < sizeX_-1;i++) inV[i+1] = aux[i];
}
...
void pdeSABR::oneStepForwardN(vector<double>&inV)
{
	...
	for(int i=0; i<=sizeX_+1; i++)
	{
		...
		if (i == 0) 
			...
			c[i] = ndt*Em_[i]*Cm_[i]*(1.0/(Fm_[i+1]-Fm_[i])+1.0/(Fm_[i]-Fm_[i-1]));
	}
	TriDiagonalSolve(sizeX_,l,c,u,d,inV);	
	inV[0] = inV[sizeX_+1]=0;
	QL_+=dT_*Cm_[1]/(Fm_[1]-Fm_[0])*Em_[1]*inV[1];
	QR_+=dT_*Cm_[sizeX_]/(Fm_[sizeX_+1]-Fm_[sizeX_])*Em_[sizeX_]*inV[sizeX_];
};
...
//			d[r-1] = inV[r]+ndt*(-3*Mgrid_[r-1]*inV[r]+Mgrid_[r]*inV[r+1]);
//		}
//		else if (r==sizeX_) 
//		{
//			u[r-1]=0;
...
//			c[r-1] = 3*ndt*Mgrid_[r-1]+1.0;
//			d[r-1] = inV[r]+ndt*(Mgrid_[r-2]*inV[r-1]-3*Mgrid_[r-1]*inV[r]);
//		}
//		else
//		{
//			d[r-1] = inV[r]+ndt*(Mgrid_[r-2]*inV[r-1]-2*Mgrid_[r-1]*inV[r]+Mgrid_[r]*inV[r+1]);
//			l[r-1] = -ndt*Mgrid_[r-2];
...
vector<double> pdeSABR::calculateDensity()
{
	...
	setFbounds();
	pde_func Mf;
	if (type_=="Normal") Mf = boost::bind(&pdeSABR::Mnormal, this, _1,_2);
	else Mf = boost::bind(&pdeSABR::Mshifted, this, _1,_2);
	vector<double> inV(sizeX_+2,0.0);
	inV[int((f_-Fmin_)/h_+0.5)]=1/h_;
	for (int t = 0; t<sizeT_; t++)	oneStepForward(t*dT_,inV,Mf);
	return inV;
};
void pdeSABR::setDensity()
{
	Density_ = calculateDensity();
}
...
void pdeSABR::setFbounds()
{
	...
	Fmax_ = pow(fup*eps_*alpha_*(1-beta_)+pow(f_+shift_,1-beta_),1.0/(1-beta_))-shift_;
	Fmin_ = pow(-fdn*eps_*alpha_*(1-beta_)+pow(f_+shift_,1-beta_),1.0/(1-beta_))-shift_;
	if (Fmin_>=Fmax_) throw("Error in grid Bounds!");
	if (f_<Fmin_) throw("Bad grid bounds %3.2f < %3.2f", f_ , Fmin_);
	if (f_>Fmax_) throw("Bad grid bounds %3.2f < %3.2f", Fmax_ , f_);
	if ((Fmin_!=Fmin_)||(Fmax_!=Fmax_)) throw("Illegal bounds, ajust shift and recalibrate");
...
{
	double payoff=0.0;
	if (Density_.size()<=1) setDensity();
	if (type=="Call")
	{
		if (strike < Fmin_) payoff = f_-strike;
		else if((strike <= Fmax_)&&(strike >= Fmin_))
		{
			int i = 0;
			for( i = 1; i <= sizeX_; i ++) if ( (Fgrid_[i]>=strike) && (Fgrid_[i+1]<strike)) break;
			do
			{
	...
	{
		if (strike > Fmax_) payoff = f_-strike;
