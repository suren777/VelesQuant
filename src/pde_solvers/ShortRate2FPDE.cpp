...
#include <velesquant/models/triDiagMatrix.h>
...
static const double Wxy[Mxy] = {
	...
	-4.0000,-3.6480,-3.3269,-3.0341,-2.7670,-2.5234,-2.3013,-2.0986,-1.9138,-1.7453,-1.5916,-1.4513,-1.3234,-1.2067,-1.1003,-1.0032,-0.9147,-0.8339,-0.7601,-0.6929,-0.6315,-0.5754,-0.5243,-0.4776,-0.4350,-0.3960,-0.3604,-0.3279,-0.2982,-0.2710,-0.2461,-0.2232,-0.2023,-0.1831,-0.1654,-0.1492,-0.1342,-0.1204,-0.1076,-0.0956,-0.0846,-0.0742,-0.0644,-0.0552,-0.0465,-0.0382,-0.0301,-0.0224,-0.0148,-0.0074,0.0000,0.0074,0.0148,0.0224,0.0301,0.0382,0.0465,0.0552,0.0644,0.0742,0.0846,0.0956,0.1076,0.1204,0.1342,0.1492,0.1654,0.1831,0.2023,0.2232,0.2461,0.2710,0.2982,0.3279,0.3604,0.3960,0.4350,0.4776,0.5243,0.5754,0.6315,0.6929,0.7601,0.8339,0.9147,1.0032,1.1003,1.2067,1.3234,1.4513,1.5916,1.7453,1.9138,2.0986,2.3013,2.5234,2.7670,3.0341,3.3269,3.6480,4.0000
};
...
void ShortRate2FPDE::buildGrid(double Time, int Nt)
{
	...
	for(int n=0; n<nP; n++)
	{
		if(n==0) 
			...
			avgSigma += sigma1s_[n]*sigma1s_[n]*(timeSigma1s_[n]-timeSigma1s_[n-1]);
	}
	avgSigma = sqrt(avgSigma/timeSigma1s_[nP-1]) *sqrt((1-exp(-2*kappa1_*Time))/(2*kappa1_));
	for(int x=0; x<Mxy; x++)
	...
	for(int n=0; n<nP; n++)
	{
		if(n==0) 
			...
			avgSigma += sigma2s_[n]*sigma2s_[n]*(timeSigma2s_[n]-timeSigma2s_[n-1]);
	}
	avgSigma = sqrt(avgSigma/timeSigma2s_[nP-1]) *sqrt((1-exp(-2*kappa2_*Time))/(2*kappa2_));
	for(int x=0; x<Mxy; x++)
...
double ShortRate2FPDE::pricingSwaption(double Expiry, double Tenor, double Strike, double PayFrequency)
{
	...
	QL_ENSURE(swaptionValue >= -0.0001, "pricingSwaption Functor Fails " << swaptionValue);
	return swaptionValue;
};

void ShortRate2FPDE::oneStepBackwardADIDouglas(int t, const vector<vector<double> >&inM, vector<vector<double> >&outM)
{
...
void ShortRate2FPDE::oneStepBackwardExplicit(int t, const vector<vector<double> >&inM, vector<vector<double> >&outM)
{
	...
	outM[0][0] = (1.0/delT-rate)*inM[0][0]
	...
	for(int k=1; k<Mxy-1; k++)
	{
		...
		rate = alpha+X0+Y;
		outM[0][k] = (1.0/delT-rate)*inM[0][k] 
					+(conv1*(X2+X1-2*X0)+difu1)/(X1-X0)/(X2-X0)*inM[0][k] 
	...
	rate = alpha+X0+Y1l;
	outM[0][Mxy-1] = (1.0/delT-rate)*inM[0][Mxy-1] 
					+(conv1*(X2+X1-2*X0)+difu1)/(X1-X0)/(X2-X0)*inM[0][Mxy-1] 
					+(-conv1*(X2-X0)-difu1)/(X1-X0)/(X2-X1)*inM[1][Mxy-1]
					+(conv1*(X1-X0)+difu1)/(X2-X1)/(X2-X0)*inM[2][Mxy-1]
					+(-conv2*(Y1l-Y2l)+difu2)/(Y2l-Y3l)/(Y1l-Y3l)*inM[0][Mxy-3] 
					+(conv2*(Y1l-Y3l)-difu2)/(Y2l-Y3l)/(Y1l-Y2l)*inM[0][Mxy-2]
					+(-conv2*(2*Y1l-Y2l-Y3l)+difu2)/(Y1l-Y2l)/(Y1l-Y3l)*inM[0][Mxy-1]; 
	...
	for(int j=1; j<Mxy-1; j++)
	{
		...
		outM[j][Mxy-1] = (1.0/delT-rate)*inM[j][Mxy-1] 
						+(conv1*(Xu-X)+difu1)/(X-Xl)/(Xu-Xl)*inM[j-1][Mxy-1]
						+(-conv1*(Xu-2*X+Xl)-difu1)/(X-Xl)/(Xu-X)*inM[j][Mxy-1] 
						+(-conv1*(X-Xl)+difu1)/(Xu-X)/(Xu-Xl)*inM[j+1][Mxy-1]
						+(-conv2*(Y1l-Y2l)+difu2)/(Y2l-Y3l)/(Y1l-Y3l)*inM[j][Mxy-3] 
						+(conv2*(Y1l-Y3l)-difu2)/(Y2l-Y3l)/(Y1l-Y2l)*inM[j][Mxy-2]
						+(-conv2*(2*Y1l-Y2l-Y3l)+difu2)/(Y1l-Y2l)/(Y1l-Y3l)*inM[j][Mxy-1]; 
	...
	rate = alpha+X1l+Y0;
	outM[Mxy-1][0] = (1.0/delT-rate)*inM[Mxy-1][0] 
				+(-conv1*(X1l-X2l)+difu1)/(X2l-X3l)/(X1l-X3l)*inM[Mxy-3][0]
				+(conv1*(X1l-X3l)-difu1)/(X2l-X3l)/(X1l-X2l)*inM[Mxy-2][0]
				+(-conv1*(2*X1l-X2l-X3l)+difu1)/(X1l-X2l)/(X1l-X3l)*inM[Mxy-1][0]
				+(conv2*(Y2+Y1-2*Y0)+difu2)/(Y1-Y0)/(Y2-Y0)*inM[Mxy-1][0] 
	...
	for(int k=1; k<Mxy-1; k++)
	{
		...
		outM[Mxy-1][k] = (1.0/delT-rate)*inM[Mxy-1][k] 
					+(-conv1*(X1l-X2l)+difu1)/(X2l-X3l)/(X1l-X3l)*inM[Mxy-3][k]
					+(conv1*(X1l-X3l)-difu1)/(X2l-X3l)/(X1l-X2l)*inM[Mxy-2][k]
					+(-conv1*(2*X1l-X2l-X3l)+difu1)/(X1l-X2l)/(X1l-X3l)*inM[Mxy-1][k]
					+(conv2*(Yu-Y)+difu2)/(Y-Yl)/(Yu-Yl)*inM[Mxy-1][k-1]
					+(-conv2*(Yu-2*Y+Yl)-difu2)/(Y-Yl)/(Yu-Y)*inM[Mxy-1][k] 
	...
	rate = alpha+X1l+Y1l;
	outM[Mxy-1][Mxy-1] = (1.0/delT-rate)*inM[Mxy-1][Mxy-1]
					+(-conv1*(X1l-X2l)+difu1)/(X2l-X3l)/(X1l-X3l)*inM[Mxy-3][Mxy-1]
					+(conv1*(X1l-X3l)-difu1)/(X2l-X3l)/(X1l-X2l)*inM[Mxy-2][Mxy-1]
					+(-conv1*(2*X1l-X2l-X3l)+difu1)/(X1l-X2l)/(X1l-X3l)*inM[Mxy-1][Mxy-1]
					+(-conv2*(Y1l-Y2l)+difu2)/(Y2l-Y3l)/(Y1l-Y3l)*inM[Mxy-1][Mxy-3] 
					+(conv2*(Y1l-Y3l)-difu2)/(Y2l-Y3l)/(Y1l-Y2l)*inM[Mxy-1][Mxy-2]
					+(-conv2*(2*Y1l-Y2l-Y3l)+difu2)/(Y1l-Y2l)/(Y1l-Y3l)*inM[Mxy-1][Mxy-1]; 
	outM[Mxy-1][Mxy-1] = delT*outM[Mxy-1][Mxy-1];
};
void ShortRate2FPDE::oneStepBackwardDouglasX(int t, const vector<vector<double> >&inM, const vector<vector<double> >&midM, vector<vector<double> >&outM)
{
	...
	for(int k=0; k<Mxy; k++)
	{
		...
		for(int j=1; j<Mxy-1; j++)
		{
			...
			c[j-1] = 1.0/delT+rate+(conv1*(Xu-2.0*X+Xl)+difu1)/(X-Xl)/(Xu-X);
			u[j-1] = (conv1*(X-Xl)-difu1)/(Xu-X)/(Xu-Xl);
			d[j-1] = (-conv1*(Xu-X)-difu1)/(X-Xl)/(Xu-Xl)*inM[j-1][k]
					+(rate+(conv1*(Xu-2.0*X+Xl)+difu1)/(X-Xl)/(Xu-X))*inM[j][k]
					+(conv1*(X-Xl)-difu1)/(Xu-X)/(Xu-Xl)*inM[j+1][k]
				    +midM[j][k]/delT;
		}
		...
		double k0 = 1.0/delT+rate+(-conv1*(X2+X1-2.0*X0)-difu1)/(X1-X0)/(X2-X0);
		double k1 = (conv1*(X2-X0)+difu1)/(X1-X0)/(X2-X1);
		double k2 = (-conv1*(X1-X0)-difu1)/(X2-X1)/(X2-X0);
		double d0 = (rate+(-conv1*(X2+X1-2.0*X0)-difu1)/(X1-X0)/(X2-X0))*inM[0][k]
				   +(conv1*(X2-X0)+difu1)/(X1-X0)/(X2-X1)*inM[1][k]
				   +(-conv1*(X1-X0)-difu1)/(X2-X1)/(X2-X0)*inM[2][k]
				   +midM[0][k]/delT;
		c[0] = c[0] -l[0]*k1/k0;
		...
		double kl1 = 1.0/delT+rate+(conv1*(2.0*X1-X2-X3)-difu1)/(X1-X2)/(X1-X3);
		double dl1 = (conv1*(X1-X2)-difu1)/(X2-X3)/(X1-X3)*inM[Mxy-3][k]
					+(-conv1*(X1-X3)+difu1)/(X2-X3)/(X1-X2)*inM[Mxy-2][k]
					+(rate+(conv1*(2.0*X1-X2-X3)-difu1)/(X1-X2)/(X1-X3))*inM[Mxy-1][k]
					+midM[Mxy-1][k]/delT;
		...
		outM[Mxy-1][k] = (dl1 -kl3*outM[Mxy-3][k] -kl2*outM[Mxy-2][k])/kl1;
	}
};
void ShortRate2FPDE::oneStepBackwardDouglasY(int t, const vector<vector<double> >&inM, const vector<vector<double> >&midM, vector<vector<double> >&outM)
{
	vector<double> l(Mxy-2), c(Mxy-2), u(Mxy-2), d(Mxy-2), V(Mxy-2);
	double te = gridT_[t+1];
	double ti = gridT_[t];
	...
	for(int j=0; j<Mxy; j++)
	{
		...
		for(int k=1; k<Mxy-1; k++)
		{
			...
			c[k-1] = 1.0/delT+rate+(conv2*(Yu-2.0*Y+Yl)+difu2)/(Y-Yl)/(Yu-Y);
			u[k-1] = (conv2*(Y-Yl)-difu2)/(Yu-Y)/(Yu-Yl);
			d[k-1] = (-conv2*(Yu-Y)-difu2)/(Y-Yl)/(Yu-Yl)*inM[j][k-1]
					+(rate+(conv2*(Yu-2.0*Y+Yl)+difu2)/(Y-Yl)/(Yu-Y))*inM[j][k]	
					+(conv2*(Y-Yl)-difu2)/(Yu-Y)/(Yu-Yl)*inM[j][k+1]
				    +midM[j][k]/delT;
		}	
		...
		double k0 = 1.0/delT+rate+(-conv2*(Y2+Y1-2.0*Y0)-difu2)/(Y1-Y0)/(Y2-Y0);
		double k1 = (conv2*(Y2-Y0)+difu2)/(Y1-Y0)/(Y2-Y1);
		double k2 = (-conv2*(Y1-Y0)-difu2)/(Y2-Y1)/(Y2-Y0);
		double d0 = (rate+(-conv2*(Y2+Y1-2.0*Y0)-difu2)/(Y1-Y0)/(Y2-Y0))*inM[j][0]
				   +(conv2*(Y2-Y0)+difu2)/(Y1-Y0)/(Y2-Y1)*inM[j][1]
				   +(-conv2*(Y1-Y0)-difu2)/(Y2-Y1)/(Y2-Y0)*inM[j][2]
				   +midM[j][0]/delT;
		c[0] = c[0] -l[0]*k1/k0;
		...
		double kl1 = 1.0/delT+rate+(conv2*(2.0*Y1-Y2-Y3)-difu2)/(Y1-Y2)/(Y1-Y3);
		double dl1 = (conv2*(Y1-Y2)-difu2)/(Y2-Y3)/(Y1-Y3)*inM[j][Mxy-3]
					+(-conv2*(Y1-Y3)+difu2)/(Y2-Y3)/(Y1-Y2)*inM[j][Mxy-2]
					+(rate+(conv2*(2.0*Y1-Y2-Y3)-difu2)/(Y1-Y2)/(Y1-Y3))*inM[j][Mxy-1]
					+midM[j][Mxy-1]/delT;
		l[Mxy-3] = l[Mxy-3] -u[Mxy-3]*kl3/kl1;
		c[Mxy-3] = c[Mxy-3] -u[Mxy-3]*kl2/kl1;
		d[Mxy-3] = d[Mxy-3] -u[Mxy-3]*dl1/kl1;
		...
		outM[j][Mxy-1] = (dl1 -kl3*outM[j][Mxy-3] -kl2*outM[j][Mxy-2])/kl1;
	}
};

void ShortRate2FPDE::calibrator(vector<double> timeDFs, vector<double> DFs, vector<defSwap> swapQuotes)
{
	DFs_ = DFs;
	...
	boost::function<void (ShortRate2FPDE*, int,int,double*,double*,int*)> obj = &ShortRate2FPDE::objFcnCalibrator;
...
void ShortRate2FPDE::objFcnCalibrator(int m, int n, double* x, double* fvec, int* iflag)
{
    int ns1 = sigma1s_.size();
    int ns2 = sigma2s_.size();
	for(int i=0; i<ns1; i++)
	...
	for(int i=0; i<m; i++)
	{
		double model = pricingSwaption(quoteSwap_[i].Expiry, quoteSwap_[i].Tenor, quoteSwap_[i].SwapRate, quoteSwap_[i].Frequency);
...
void ShortRate2FPDE::termStructureCalibrator()
{
	...
	for(int x=1; x<Mxy-1; x++)
		if(gridX_[x] >= 0.0)
		{
			for(int y=1; y<Mxy-1; y++)
				if(gridY_[y] >= 0.0)
				{
					inV[x][y] = 4.0/((gridX_[x+1]-gridX_[x-1])*(gridY_[y+1]-gridY_[y-1])); // Dirac delta as initial distribution 
					break;
				}
			break;
		}
	...
	for(int i=0; i<n; i++)
	{
		...
		do {
			...
			if(alphas_[i] == 0.0) alphas_[i] = 0.0005;	// case of 0 value handler
			...
			alphas_[i] *= (1 + 0.001*(DFs_[i]-df)/(dfUP-df));	// Newton iteration
			alphas_[i] = max(-0.0050, alphas_[i]);	// Cutoff the nagative value (BETTER)
	...
	for(int x=1; x<Mxy-1; x++)
		if(gridX_[x] >= 0.0)
		{
			for(int y=1; y<Mxy-1; y++)
				if(gridY_[y] >= 0.0)
				{
					//inV[x][y] = 4.0/((gridX_[x+1]-gridX_[x])*(gridY_[y+1]-gridY_[y])+(gridX_[x+1]-gridX_[x])*(gridY_[y]-gridY_[y-1])
					//	            +(gridX_[x]-gridX_[x-1])*(gridY_[y+1]-gridY_[y])+(gridX_[x]-gridX_[x-1])*(gridY_[y]-gridY_[y-1])); // Dirac delta as initial distribution 
					inV[x][y] = 4.0/((gridX_[x+1]-gridX_[x-1])*(gridY_[y+1]-gridY_[y-1])); // Dirac delta as initial distribution 
					break;
				}
			break;
...
double ShortRate2FPDE::trapezoidal2D(vector<vector<double> >& inV)
{
	...
	for(int x=1; x<Mxy; x++)
		for(int y=1; y<Mxy; y++)
			value += 0.25*(inV[x][y]+inV[x][y-1]+inV[x-1][y]+inV[x-1][y-1])*(gridX_[x]-gridX_[x-1])*(gridY_[y]-gridY_[y-1]);
	return value;
};

void ShortRate2FPDE::oneStepForwardADIDouglas(int T, const vector<vector<double> >&inM, vector<vector<double> >&outM)
{
...
void ShortRate2FPDE::oneStepForwardExplicit(int T, const vector<vector<double> >&inM, vector<vector<double> >&outM)
{
	double Te = gridT_[T+1];
	double Ti = gridT_[T];
	double delT = Te - Ti;
...
void ShortRate2FPDE::oneStepForwardDouglasX(int T, const vector<vector<double> >&inM, const vector<vector<double> >&midM, vector<vector<double> >&outM)
{
	vector<double> l(Mxy-2), c(Mxy-2), u(Mxy-2), d(Mxy-2), V(Mxy-2);
	double Te = gridT_[T+1];
	double Ti = gridT_[T];
...
void ShortRate2FPDE::oneStepForwardDouglasY(int T, const vector<vector<double> >&inM, const vector<vector<double> >&midM, vector<vector<double> >&outM)
{
	vector<double> l(Mxy-2), c(Mxy-2), u(Mxy-2), d(Mxy-2), V(Mxy-2);
	double Te = gridT_[T+1];
	double Ti = gridT_[T];
...
double ShortRate2FPDE::whichValue(double t, const vector<double> times, const vector<double> values)
{
	int n = times.size();
	if(t < times[0])
		return values[0];
