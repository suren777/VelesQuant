...
void sabr_pde::oneStepForward(vector<double> &inV, double dt, double &PL,
                              double &PR) {
...
void sabr_pde::calculateDensity() {
  ...
  for (int i = 0; i <= J + 1; i++)
    ym[i] = Y(i * h_ + zmin_ - 0.5 * h_);
  Fm_ = F(ym);
  Fm_[0] = 2 * F(Y(zmin_)) - Fm_[1];
  Fm_[J + 1] = 2 * F(Y(zmax_)) - Fm_[J];
  Cm_ = C(ym, Fm_);
  Cm_[0] = Cm_[1];
  Cm_[J + 1] = Cm_[J];
  ...
  for (int i = 0; i < sizeX_; i++)
    Em_[i] = 1.0;
  vector<double> Emdt1 = Emd(dt1, Gamma);
  Emdt1[0] = Emdt1[1];
  Emdt1[sizeX_ - 1] = Emdt1[sizeX_ - 2];
  vector<double> Emdt2 = Emd(dt2, Gamma);
  Emdt2[0] = Emdt2[1];
...
double afSABR::L(double F) { return pow(F + shift_, beta_); }
double afSABR::Lm(double F) { return pow(F + shift_, 1 - beta_); }
void afSABR::setZbounds() {
  ...
  if (beta_ < 1) {
    ...
    double zb = -1 / nu_ *
                log((sqrt(1 - rho_ * rho_ + pow(rho_ + nu_ * aux / alpha_, 2)) -
                     rho_ - nu_ * aux / alpha_) /
                    (1 - rho_));
...
double antonovSABR::L(double F) { return pow(abs(F), beta_); };
double antonovSABR::Lm(double F) { return pow(abs(F), 1 - beta_); };

afSABR::~afSABR() {};
antonovSABR::~antonovSABR() {};
} // namespace velesquant