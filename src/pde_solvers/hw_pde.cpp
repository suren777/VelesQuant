...
double HWPDE::pricingCBO(double Expiry, double Tenor, double Coupon,
  ...
  return -1; // If you ever hit here, you are doing something completely wrong,
             // believe me man.
};
...
double HWPDE::getSwapRate(double Expiry, double Tenor, double PayFrequency) {
  ...
  //	QL_ENSURE(SwapRate >= -0.0005, "getSwapRate Functor Fails " <<
  // SwapRate); //Negative 50BP Rate
  return SwapRate;
};
double HWPDE::swaptionValueATM(double Expiry, double Tenor, double PayFrequency,
                               double SwapRate, double VolATM) {
  double Level = getAnnuity(Expiry, Tenor, PayFrequency);
  return Level * SwapRate *
...
void HWPDE::calibrator(vector<double> timeDFs, vector<double> DFs,
  ...
  int n =
      sigmas_.size() + 1; // no. of HullWhite model volatility term paremeters
  ...
  for (int i = 0; i < m; i++)
    marketSwaption_[i] = swaptionATM(quoteSwap_[i].Expiry, quoteSwap_[i].Tenor,
                                     quoteSwap_[i].VolATM);
  ...
  boost::function<void(HWPDE *, int, int, double *, double *, int *, double *,
                       double *)>
      obj = &HWPDE::objFcnCalibration;
...
double HWPDE::pen_fun(double *x, double *lb, double *ub, int n) {
  ...
  double lambda = 1e8;
  for (int i = 0; i < n; i++)
    penalty += ((x[i] <= lb[i]) || (x[i] >= ub[i])) ? lambda * pow(x[i], 2) : 0;
  return penalty;
}
...
vector<double> HWPDE::simulationPDE(vector<double> times) const {
  int N = times.size();
...
double HWPDE::dBratio(double a, double Mi, double Tk, double Tj) {
  if (a == 0.0)
    ...
    double t1 = (Tj - Mi);
    double t2 = (Tk - Mi);
    double dF = exp(a * (t2 - t1)) *
                (t1 * (exp(t2 * a) - 1) - t2 * exp(t1 * a) + t2) /
                (exp(t2 * a) - 1.0) / (exp(t2 * a) - 1.0);
    return dF;
  }
};

double HWPDE::objKappa(double a, const vector<double> &ivr,
...
void HWPDE::calibratorBootStrap(vector<double> timeDFs, vector<double> DFs,
                                vector<defSwap> swapQuotes) {
  QL_REQUIRE(!swapQuotes.empty(), "Swap quotes cannot be empty");
  QL_REQUIRE(!timeDFs.empty(), "Time DFs cannot be empty");
  int MAX_ITER = 50;
  ...
  for (int j = 0; j < timeSigmas_.size(); j++) {
    ...
    for (int i = 0; i < m; i++)
      marketSwaption_[i] =
          swaptionValueATM(qSB_[i].Expiry, qSB_[i].Tenor, qSB_[i].Frequency,
                           qSB_[i].SwapRate, qSB_[i].VolATM);
...
double HWPDE::objFcnCalibrationB(double x, int m) {
  ...
  termStructureCalibratorBtstrp(
      qSB_[m - 1].Expiry + qSB_[m - 1].Tenor); // TEREM STRUCTURE CALIBRATION
  double sum = 0.0;
  for (int i = 0; i < m; i++) {
    double modelSwaption = pricingSwaption(qSB_[i].Expiry, qSB_[i].Tenor,
...
double HWPDE::swaptionIVblack(double Expiry, double Tenor,
...
void HWPDE::termStructureCalibratorBtstrp(double max_maturity) {
  int N = 0;
  if (timeThetas_[0] > max_maturity)
    N = 1;
  else
    for (int i = 1; i < timeThetas_.size(); i++)
      if ((max_maturity > timeThetas_[i - 1]) &&
          (max_maturity <= timeThetas_[i])) {
        N = i;
        break;
      }
  std::vector<int> iTs(N);
  ...
  for (int i = 0; i < N; i++) {
    ...
    do {
      ...
      df = trapezoidal(inV);
      thetas_[i] *= (1 + 0.001 * (getDFinterp(timeThetas_[i]) - df) /
                             (dfUP - df)); // Newton iteration
      thetas_[i] =
          max(-0.0050, thetas_[i]); // Cutoff the nagative value (BETTER)
    } while (fabs(1.0 - df / getDFinterp(timeThetas_[i])) >= 1.0E-6 &&
             niter < 7);
    inV = lastV; // reset to the starting density
    for (int t = sT; t < iTs[i] - 1; t++)
      oneStepForward(t, inV);
